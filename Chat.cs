    ...
    private static void Update()
    {
      ...
      while (true)
      {
        while (socket?.Connected == true)
        {
                  ...
                  else if (message[0].StartsWith("@") && message[0].EndsWith("PRIVMSG"))
                  {
                    ...
                    else if (ResponseMessages.Count > 0) // Check if message starts with key to get automatic response
                    {
                      ...
                      if (ResponseMessages.TryGetValue(temp, out dictionaryResponse))
                      {
                        ...
                        {
                          socket.Send(Encoding.UTF8.GetBytes($"PRIVMSG #{Config.Data[Config.Keys.ChannelName]} :@{userName} {dictionaryResponse.Item1}\r\n"));
          ...
          // Additional sending of messages
          {
            if (MessageQueue.Count > 0)
            {
              lock (MessageQueue)
              {
                socket.Send(Encoding.UTF8.GetBytes($"PRIVMSG #{Config.Data[Config.Keys.ChannelName]} :{MessageQueue[0]}\r\n"));
            ...
            else if ((PeriodicMessages.Count > 0) && (DateTime.Now - LastPeriodicMessage >= PeriodicMessageInterval))
            {
              ...
              socket.Send(Encoding.UTF8.GetBytes($"PRIVMSG #{Config.Data[Config.Keys.ChannelName]} :{PeriodicMessages[PeriodicMessageIndex]}\r\n"));
